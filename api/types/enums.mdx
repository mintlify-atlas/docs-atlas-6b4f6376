...
Prowl.Quill exposes four public enumerations that control how paths are filled and stroked. `BrushType` and `WindingMode` live in `Prowl.Quill`; `JointStyle` and `EndCapStyle` are defined in `Prowl.Vector` and used by the canvas stroke API. Understanding each value is essential for producing correct anti-aliased output from the canvas.
...
`JointStyle` (from `Prowl.Vector`) controls how two consecutive stroke segments
...
`EndCapStyle` (from `Prowl.Vector`) controls how the two open ends of a stroked
